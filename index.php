...
<body>
    ...
    echo "<h1 id='title'>Welcome to E.P.W.F. $username<a href='https://github.com/MADMAN-Modding/EPWF' target='_blank'><img class='github' src='https://github.com/favicon.ico'></a></h1>"
    ?>
    ...
    if ($consoleFinder = opendir('roms/')) {
        ...
        foreach ($consoleList as $consoleName) {
            ...
            if ($romFinder = opendir("roms/$consoleName")) {
                ...
                echo "<style>.$consoleName{margin-left: auto; margin-right: auto;}.$consoleName-link {font-size: 20px; display: none; margin-bottom: 10px;}.$consoleName:hover .$consoleName-link {display: block; margin-left: auto; margin-right: auto;} .$consoleName {max-width: 200px;}.$consoleName:hover {max-width: 750px;}</style>";
            ...
            foreach ($romList as $romName) {
                ...
                echo "<a class=$consoleName-link href=game.html?rom=$romName&console=$consoleName&name=$username>";

                // Lists of extensions that should be removed from the display name
                $extensionsList = array("sfc", $consoleName, "7z", "rar", "zip");

                foreach ($extensionsList as $extensionListEntry) {
                    $romName = str_replace(".$extensionListEntry", "", $romName);
                }
